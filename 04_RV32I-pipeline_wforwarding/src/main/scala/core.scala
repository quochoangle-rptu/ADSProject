...
class PipelinedRV32Icore(BinaryFile: String) extends Module {
  ...
  val exmemVal = exBarrier.io.outAluResult
  val exmemRd  = exBarrier.io.outRD
  val exmemWe  = exBarrier.io.outRegWrite

  val memwbVal = memBarrier.io.outAluResult
  val memwbRd  = memBarrier.io.outRD
  val memwbWe  = memBarrier.io.outRegWrite

  val rs1_ex = idBarrier.io.outRS1
  val rs2_ex = idBarrier.io.outRS2
  val opBIsImm_ex = idBarrier.io.outOpBIsImm

  // Forwarded operands
  val fwdA = Wire(UInt(32.W))
  val fwdB = Wire(UInt(32.W))
  ...
  fwdA := idBarrier.io.outOperandA
  when(exmemWe && (exmemRd =/= 0.U) && (exmemRd === rs1_ex)) {
    fwdA := exmemVal
  }.elsewhen(memwbWe && (memwbRd =/= 0.U) && (memwbRd === rs1_ex)) {
    fwdA := memwbVal
  }

  fwdB := idBarrier.io.outOperandB
  when(!opBIsImm_ex) {
    when(exmemWe && (exmemRd =/= 0.U) && (exmemRd === rs2_ex)) {
      fwdB := exmemVal
    }.elsewhen(memwbWe && (memwbRd =/= 0.U) && (memwbRd === rs2_ex)) {
      fwdB := memwbVal
    }
  }
