...
"RV32I_RAWHazardForwarding" should "resolve read-after-write data hazards correctly" in {
  test(new PipelinedRV32I("src/test/programs/BinaryFile_pipelined_forwarding")).withAnnotations(Seq(WriteVcdAnnotation)) { dut =>

    dut.clock.setTimeout(0)

    println("=== RAW HAZARD FORWARDING TESTS ===")
    println("Testing if Forwarding Unit correctly resolves Read-After-Write hazards")
    println("")

    // Wait for pipeline to fill (5 stages)
    dut.clock.step(5)
    dut.io.result.expect(0.U)
    println("PASS: Pipeline startup -> NOP = 0")

    // =========================================================================
    // TEST 1: EX/MEM Forwarding (1-cycle hazard)
    // =========================================================================
    println("\n--- TEST 1: EX/MEM Forwarding (1-cycle RAW Hazard) ---")
...
// TEST 1: EX/MEM Forwarding (1-cycle hazard)
    ...
    dut.io.exception.expect(false.B)
    println("PASS: ADDI x1, x0, 10 -> x1 = 10")

    dut.clock.step(1)
    dut.io.result.expect(10.U)
    dut.io.exception.expect(false.B)
    println("PASS: ADD x2, x1, x0 -> x2 = 10 (EX/MEM forwarding)")
    println("     Data forwarded from previous cycle's result (x1=10)")

    // =========================================================================
    // TEST 2: MEM/WB Forwarding (2-cycle hazard)
    // =========================================================================
    println("\n--- TEST 2: MEM/WB Forwarding (2-cycle RAW Hazard) ---")

    // ADD x3, x2, x0 (uses x2 written 2 cycles ago)
    // WITHOUT forwarding: would need delay to read old x2 value
    // WITH forwarding: x2 is forwarded from MEM/WB stage
    dut.clock.step(1)
    dut.io.result.expect(10.U)
    dut.io.exception.expect(false.B)
    println("PASS: ADD x3, x2, x0 -> x3 = 10 (MEM/WB forwarding)")
    println("     Data forwarded from 2 cycles ago (x2=10)")

    // =========================================================================
    // TEST 3: Multiple Register Hazards (operandA and operandB)
    // =========================================================================
    println("\n--- TEST 3: Multiple Register Hazards ---")

    dut.clock.step(1)
    dut.io.result.expect(20.U)
    dut.io.exception.expect(false.B)
    println("PASS: ADDI x4, x0, 20 -> x4 = 20")

    // ADD x5, x2, x4
    // Hazard on x4 (operandB): x4 written in PREVIOUS cycle
    // x2 is from register file (written long ago)
    dut.clock.step(1)
    dut.io.result.expect(30.U)  // 10 + 20 = 30
    dut.io.exception.expect(false.B)
    println("PASS: ADD x5, x2, x4 -> x5 = 30 (x4 forwarded, x2 from RF)")
    println("     x4 forwarded from EX/MEM, x2 from Register File")

    dut.clock.step(1)
    dut.io.result.expect(5.U)
    dut.io.exception.expect(false.B)
    println("PASS: ADDI x6, x0, 5 -> x6 = 5")

    // =========================================================================
    // TEST 4: Both Operands Forwarded (operandA and operandB)
    // =========================================================================
    println("\n--- TEST 4: Both Operands Forwarded ---")

    // ADD x7, x5, x6
    // Hazard on x5 (operandA): x5 written in PREVIOUS cycle
    // Hazard on x6 (operandB): x6 written in PREVIOUS cycle
    dut.clock.step(1)
    dut.io.result.expect(35.U)  // 30 + 5 = 35
    dut.io.exception.expect(false.B)
    println("PASS: ADD x7, x5, x6 -> x7 = 35 (both operands forwarded)")
    println("     Both x5 and x6 forwarded from EX/MEM stage correctly")

    dut.clock.step(1)
    dut.io.result.expect(8.U)
    dut.io.exception.expect(false.B)
    println("PASS: ADDI x8, x0, 8 -> x8 = 8")

    // =========================================================================
    // TEST 5: SUB with Forwarding (Negative Result)
    // =========================================================================
    println("\n--- TEST 5: SUB with Forwarding ---")

    // SUB x9, x7, x8
    // Hazard on x7 (operandA): x7 written in PREVIOUS cycle
    // Hazard on x8 (operandB): x8 written in PREVIOUS cycle
    dut.clock.step(1)
    dut.io.result.expect(27.U)  // 35 - 8 = 27
    dut.io.exception.expect(false.B)
    println("PASS: SUB x9, x7, x8 -> x9 = 27 (both operands forwarded)")
    println("     x7 and x8 forwarded to produce correct subtraction result")

    // =========================================================================
    // TEST 6: Extended Forwarding Chain (x9 -> x10)
    // =========================================================================
    println("\n--- TEST 6: Extended Forwarding Chain ---")

    // ADD x10, x9, x0
    // Hazard on x9 (operandA): x9 written in PREVIOUS cycle
    dut.clock.step(1)
    dut.io.result.expect(27.U)
    dut.io.exception.expect(false.B)
    println("PASS: ADD x10, x9, x0 -> x10 = 27 (x9 immediate forwarded)")
    println("     x9 value forwarded allows immediate use in next instruction")

    println("\n=== ALL RAW HAZARD FORWARDING TESTS PASSED ===")
    println("✓ EX/MEM Forwarding (1-cycle hazards) working correctly")
    println("✓ MEM/WB Forwarding (2-cycle hazards) working correctly")
    println("✓ Multiple register hazards handled correctly")
    println("✓ Both operands forwarded simultaneously works")
    println("✓ Forwarding unit resolves all RAW hazards!\n")
  }
}
}
